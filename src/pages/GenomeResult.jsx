...
const GENOME_MAP = [
  {
    ...
    insight: 'Kamu berpikir sistematis dan suka menganalisis data sebelum memutuskan. Ini aset besar di era berbasis data.'
  },
  {
    key: 'leadership', label: 'Leadership', emoji: '👑', color: '#F48FB1',
    insight: 'Kamu punya insting untuk memimpin dan mengarahkan orang lain. Tim yang kamu pimpin cenderung punya arah yang jelas.'
  },
  {
    key: 'builder', label: 'Builder', emoji: '⚙️', color: '#25D366',
    insight: 'Kamu suka membangun sesuatu dari nol dan mengeksekusi ide. Gap antara konsep dan produk terasa kecil di tanganmu.'
  },
  {
    key: 'creator', label: 'Creator', emoji: '🎨', color: '#FFB74D',
    insight: 'Kamu nyaman dengan ambiguitas dan selalu punya sudut pandang segar. Kreativitasmu bukan sekadar estetika — tapi problem-solving.'
  },
  {
    key: 'communication', label: 'Communication', emoji: '💬', color: '#CE93D8',
    insight: 'Kamu mampu menyederhanakan hal kompleks dan membuat orang mengerti. Skill ini makin langka dan makin berharga.'
  },
  {
    key: 'risk_taking', label: 'Risk Taking', emoji: '🚀', color: '#EF9A9A',
    insight: 'Kamu nyaman mencoba hal baru dibanding kebanyakan orang. Toleransi risikomu tinggi — modal utama builder dan founder.'
  },
]
    ...
    <div style={{ minHeight: '100vh', background: '#0a0f0d', fontFamily: "'Plus Jakarta Sans', sans-serif", overflowX: 'hidden' }}>
      ...
      <div style={{ background: 'rgba(255,255,255,0.03)', borderBottom: '1px solid rgba(255,255,255,0.07)', padding: '14px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 10, backdropFilter: 'blur(8px)' }}>
        <button onClick={() => navigate('/discovery')}
          style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.45)', cursor: 'pointer', fontSize: '0.82rem' }}>← Ulangi</button>
        <span style={{ color: '#fff', fontWeight: 700, fontSize: '0.9rem' }}>Career DNA Kamu</span>
        <div style={{ width: 60 }} />
      </div>

      <div style={{ padding: '20px 18px 140px', maxWidth: 480, margin: '0 auto' }}>

        {/* ── 1. HERO: Kekuatan Utama ── */}
        ...
        {/* ── 2. CAREER READINESS (dengan breakdown) ── */}
        <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: '18px', marginBottom: 16, ...fade(0.07) }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
            <div>
              ...
              <button
                ...
                style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.3)', fontSize: '0.7rem', cursor: 'pointer', padding: 0 }}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.07)', borderRadius: 99, height: 8, overflow: 'hidden', marginBottom: showReadiness ? 14 : 0 }}>
            <div style={{
              ...
              width: revealed ? `${readiness}%` : '0%', height: '100%', borderRadius: 99,
          ...
          {showReadiness && (
            <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ color: 'rgba(255,255,255,0.35)', fontSize: '0.68rem', letterSpacing: '1px', marginBottom: 2 }}>DIHITUNG DARI:</div>
              {READINESS_FACTORS.map((f, i) => {
                // Distribusikan readiness ke 5 faktor dengan slight variation
                const factorVal = Math.max(10, Math.min(100, Math.round(readiness * [0.85, 0.75, 1.05, 0.95, 0.9][i])))
                return (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <div style={{ color: 'rgba(255,255,255,0.65)', fontSize: '0.78rem', fontWeight: 600 }}>✓ {f.label}</div>
                      <div style={{ color: 'rgba(255,255,255,0.3)', fontSize: '0.68rem' }}>{f.desc}</div>
                    </div>
                    <div style={{ color: '#34B7F1', fontWeight: 700, fontSize: '0.82rem', flexShrink: 0, marginLeft: 12 }}>{factorVal}%</div>
                  </div>
                )
              })}
            </div>
          ...
          {growth.career_stage && (
            <div style={{ marginTop: 10, color: 'rgba(255,255,255,0.3)', fontSize: '0.72rem' }}>
        ...
        {/* ── 3. CAREER GENOME (dengan insight per skor) ── */}
        <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: '18px', marginBottom: 16, ...fade(0.12) }}>
          ...
          {sortedGenome.map((g, i) => {
            ...
            const isExpanded = expandGenome === g.key
            return (
              <div key={g.key} style={{ marginBottom: 14 }}>
                <div
                  onClick={() => setExpandGenome(isExpanded ? null : g.key)}
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <span style={{ color: 'rgba(255,255,255,0.2)', fontSize: '0.7rem' }}>{isExpanded ? '▲' : '▾'}</span>
                    </div>
                  </div>
                ...
                {isExpanded && (
                  <div style={{
                    ...
                    background: `${g.color}11`, border: `1px solid ${g.color}22`,
                    borderRadius: 9, fontSize: '0.8rem', color: 'rgba(255,255,255,0.6)', lineHeight: 1.6,
                  }}>
                    {g.insight}
                  </div>
                )}
          ...
          <div style={{ color: 'rgba(255,255,255,0.25)', fontSize: '0.7rem', marginTop: 4 }}>
            Klik nama untuk lihat insight
          </div>
        </div>

        {/* ── 4. CAREER GAP ANALYSIS (Gap ❌ + Strength ✅) ── */}
        <div style={{ background: 'rgba(255,183,77,0.06)', border: '1px solid rgba(255,183,77,0.18)', borderRadius: 16, padding: '18px', marginBottom: 16, ...fade(0.17) }}>
          ...
          {result.gap_summary && (
            <div style={{ color: 'rgba(255,255,255,0.58)', fontSize: '0.83rem', lineHeight: 1.65, marginBottom: 14 }}>
              {result.gap_summary}
            </div>
          )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {/* Gap Utama */}
            <div>
              <div style={{ color: 'rgba(255,255,255,0.35)', fontSize: '0.68rem', letterSpacing: '1px', marginBottom: 8 }}>GAP UTAMA:</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {gapSkills.length > 0 ? gapSkills.map((skill, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.82rem', color: 'rgba(255,255,255,0.65)' }}>
                    <span style={{ color: '#EF5350', fontWeight: 700, fontSize: '0.75rem', flexShrink: 0 }}>✕</span>
                    <span>{skill}</span>
                  </div>
                )) : (
                  <div style={{ color: 'rgba(255,255,255,0.25)', fontSize: '0.78rem' }}>Chat lebih banyak untuk deteksi gap</div>
                )}
              </div>
            </div>
            ...
            {/* Kekuatan Terbesar */}
            <div>
              <div style={{ color: 'rgba(255,255,255,0.35)', fontSize: '0.68rem', letterSpacing: '1px', marginBottom: 8 }}>KEKUATAN TERBESAR:</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {topTwo.map(g => (
                  <div key={g.key} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.82rem', color: 'rgba(255,255,255,0.65)' }}>
                    <span style={{ color: '#25D366', fontWeight: 700, fontSize: '0.75rem', flexShrink: 0 }}>✓</span>
                    <span>{g.label}</span>
                  </div>
                ))}
              </div>
        ...
        {/* ── 5. CAREER GPS PREVIEW (prominent, before paywall) ── */}
        {gpsSteps.length > 0 && (
          <div style={{ background: 'rgba(52,183,241,0.06)', border: '1px solid rgba(52,183,241,0.2)', borderRadius: 16, padding: '18px', marginBottom: 6, ...fade(0.22) }}>
            ...
            <div style={{ color: 'rgba(255,255,255,0.35)', fontSize: '0.75rem', marginBottom: 14 }}>
              Roadmap personal untuk menutup gap di atas 👆
            </div>
              ...
              {gpsSteps.map((step, i) => {
                ...
                return (
                  <div key={i} style={{
                    ...
                    background: isDone
                      ? 'rgba(37,211,102,0.1)'
                      : isFree ? 'rgba(52,183,241,0.07)' : 'rgba(255,255,255,0.02)',
                    border: isDone
                      ? '1px solid rgba(37,211,102,0.25)'
                      : isFree ? '1px solid rgba(52,183,241,0.15)' : '1px solid rgba(255,255,255,0.04)',
                    ...
                    <div style={{
                      ...
                      color: isDone ? '#25D366' : isFree ? 'rgba(255,255,255,0.82)' : 'rgba(255,255,255,0.15)',
                      flex: 1,
                    ...
                    {!isFree && (
                      <div style={{
                        ...
                        <span style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.28)', fontWeight: 600, letterSpacing: '0.5px' }}>
                          🔒 PREMIUM
                        </span>
                      </div>
                    )}
            ...
            {lockedCount > 0 && (
              <div style={{ marginTop: 10, textAlign: 'center', color: 'rgba(255,255,255,0.25)', fontSize: '0.72rem' }}>
      ...
      <div style={{
        ...
        background: 'linear-gradient(to top, #0a0f0d 78%, transparent)',
      }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginBottom: 10 }}>
          <img src="/diah-anna.png" alt="" style={{ width: 22, height: 22, borderRadius: '50%', objectFit: 'cover' }} />
          <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.75rem' }}>
            Saya sudah menyiapkan GPS khusus untuk menutup gap ini.
          </span>
        </div>
        <button
        ...
        <div style={{ textAlign: 'center', marginTop: 7, color: 'rgba(255,255,255,0.18)', fontSize: '0.68rem' }}>
