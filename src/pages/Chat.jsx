...
export default function Chat({ user, chatMessages = [], setChatMessages }) {
  ...
  // ── visualViewport: fix input bar hilang saat keyboard muncul di mobile ──
  useEffect(() => {
    const update = () => {
      if (!containerRef.current) return
      const vv = window.visualViewport
      if (vv) {
        // visualViewport.height sudah exclude keyboard — akurat di iOS & Android
        const availH = vv.height - BOTTOM_NAV_H
        containerRef.current.style.height = availH + 'px'
        containerRef.current.style.top    = vv.offsetTop + 'px'
      } else {
        // fallback browser lama
        containerRef.current.style.height = `calc(100% - ${BOTTOM_NAV_H}px)`
  ...
  const showPaywallInCoach = (feature) => {
    ...
    const msg = isLocked ? (PAYWALL_LOCKED[feature] || fallback) : (PAYWALL_EXHAUSTED[feature] || fallback)
  ...
  const showPaywall = (feature) => {
    ...
    const msg = isLocked ? (PAYWALL_LOCKED[feature] || fallback) : (PAYWALL_EXHAUSTED[feature] || fallback)
    pushBot(msg, [{ id: '__pricing', label: '⭐ Lihat Paket & Harga' }])
    setMode('coach')
  }
  ...
  const doCvReview = async (jobTarget) => {
    ...
    try {
      ...
      pushBot(data.review, [
        ...
        { id: '__share_cv', label: '📤 Bagikan hasil' },
  ...
  const doAts = async (jobDescription) => {
    ...
    try {
      ...
      pushBot(data.result, [
        ...
        { id: '__share_ats', label: '📤 Bagikan hasil' },
        { id: '__share_app', label: '👥 Ajak teman coba' },
      ])
      setCoachHistory([
        { role: 'user', content: `Tolong cek ATS score CV saya${jobDescription ? ` untuk job description: ${jobDescription.slice(0, 500)}` : ''}.` },
  ...
  return (
    <>
      {/*
        Container: position fixed, height di-set oleh visualViewport listener.
        Default: calc(100% - BOTTOM_NAV_H) — di-override JS saat keyboard naik.
      */}
      <div ref={containerRef} style={{
        ...
        width: '100%',
        maxWidth: 480,
        height: `calc(100% - ${BOTTOM_NAV_H}px)`,
        display: 'flex',
        ...
        <div style={{
          ...
          <img
            src="/diah-anna.png"
            alt="Diah Anna"
            style={{ width: 42, height: 42, borderRadius: '50%', objectFit: 'cover', flexShrink: 0, border: '2px solid rgba(37,211,102,0.4)' }}
          />
          <div style={{ flex: 1 }}>
          ...
          <button
            ...
            style={{
              color: '#fff', fontSize: '0.72rem', padding: '3px 8px',
              background: plan === 'free' ? 'rgba(255,200,0,0.25)' : 'rgba(255,255,255,0.15)',
          ...
          <button
            ...
            style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.78rem', fontWeight: 600, padding: '4px 8px', background: 'rgba(255,255,255,0.1)', borderRadius: 6, border: 'none', cursor: 'pointer' }}
          >
            Bagikan
          </button>
          <button
            onClick={handleSignOut}
            style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.78rem', padding: '4px 8px', background: 'rgba(255,255,255,0.1)', borderRadius: 6, border: 'none', cursor: 'pointer' }}
          >
            Keluar
          </button>
        </div>

        {/* ── Messages area — satu-satunya yang scroll ── */}
        <div style={{
          flex: 1,
          overflowY: 'auto',
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, padding: '5px 4px 6px' }}>
                    {msg.quickReplies.map(qr => (
                      <button
                        ...
                        disabled={loading}
                        style={{
                          background: '#fff', border: '1.5px solid #25D366', color: '#075E54',
          ...
          {loading && (
            <div style={{ display: 'flex', justifyContent: 'flex-start', marginTop: 2 }}>
              <div style={{ background: '#fff', borderRadius: '3px 14px 14px 14px', padding: '12px 16px', boxShadow: '0 1px 2px rgba(0,0,0,0.1)', display: 'flex', gap: 4, alignItems: 'center' }}>
                {[0, 1, 2].map(i => (
                  <div key={i} style={{ width: 7, height: 7, borderRadius: '50%', background: '#aaa', animation: `dot-bounce 1.2s ${i * 0.2}s infinite ease-in-out` }} />
                ))}
              </div>
            </div>
          )}
        ...
        {canUpload && (
          <div style={{
            background: '#e8f5e9', borderTop: '1px solid #c8e6c9',
            padding: '6px 14px', fontSize: '0.75rem', color: '#2e7d32',
