...
export default function Chat({ user, chatMessages = [], setChatMessages }) {
  ...
  useEffect(() => {
    ...
    if (greetingKey && sessionStorage.getItem(greetingKey)) return
...
Pilih yang sesuai buat kamu:`
  ...
  useEffect(() => {
    ...
    const onVisibilityChange = () => {
      ...
      if (document.visibilityState === 'visible') sessionSavedRef.current = false
    }
  ...
  const triggerGenomeTeaser = async () => {
    ...
    try {
      ...
      if (!gs || !Object.values(gs).some(v => (v || 0) > 0)) return
  ...
  return (
    <>
      ...
      {/* ✅ BOTTOMNAV DI-RENDER DI SINI - INI YANG HILANG DI FILE BERMASALAH */}
