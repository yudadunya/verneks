...
export default function Chat({ user, chatMessages = [], setChatMessages }) {
  ...
  const route = async (id, label) => {
    ...
    // ── Guard: kalau sedang dalam flow cv-maker, tangkap duluan sebelum apapun ──
    if (mode === 'cv-maker-upload') {
      if (id.startsWith('__') || CV_FORMATS.find(f => f.id === id)) {
        // biarkan lanjut ke switch/handler normal
    ...
    if (mode === 'cv-maker-format' && CV_FORMATS.find(f => f.id === id)) {
      await doCvMaker(id.replace('fmt_', ''))
      return
    }
