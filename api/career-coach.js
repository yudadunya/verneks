...
export default async function handler(req, res) {
  ...
  if (userId) {
    try {
      const [profileRes, growthRes, genomeRes, sessionNotesRes] = await Promise.all([
        ...
        supabase.from('user_session_notes').select('highlights, emotional_state, action_items, session_date').eq('user_id', userId).order('session_date', { ascending: false }).limit(3),
      ])
      careerProfile = profileRes.data
      growthState   = growthRes.data
      genomeData    = genomeRes.data
      sessionNotes  = sessionNotesRes.data || []
    } catch (e) {
  ...
  const sessionNotesText = sessionNotes.length > 0
    ? sessionNotes.map((n, i) =>
        `Sesi ${i+1} (${n.session_date}): ${n.highlights || '-'} | Kondisi: ${n.emotional_state || '-'} | Komitmen: ${(n.action_items||[]).join(', ')||'-'}`
      ).join('\n')
    : 'Belum ada catatan sesi sebelumnya'
