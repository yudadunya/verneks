...
const WA_LIMITS = { free: 0, starter: 10, pro: 30, platinum: 999 }
...
async function generateMessage(situation, context) {
  const situations = {
    ...
    unregistered_repeat: `User ini belum terdaftar dan sudah pernah diajak daftar sebelumnya. Ingatkan lagi dengan cara yang berbeda dan lebih personal.`,
    free_gate_first: `User ini terdaftar tapi pakai paket Free — tidak bisa WA sama saya. Ini pertama kali mereka coba WA. Beritahu dengan cara yang hangat bahwa untuk WA sama saya perlu upgrade. Tawarkan Starter (Rp 49rb/bulan). Link: lamarcerdas.my.id/pricing`,
    free_gate_repeat: `User ini pakai Free dan sudah dapat info upgrade sebelumnya tapi masih WA lagi. Ingatkan lagi dengan cara yang berbeda — lebih personal.`,
    limit_starter: `User ini pakai Starter dan sesi WA-nya bulan ini sudah habis. Beritahu dengan empathetic dan tawarkan upgrade ke Pro (Rp 199rb/bulan) atau Platinum (Rp 399rb/bulan). Link: lamarcerdas.my.id/pricing`,
    limit_pro: `User ini pakai Pro dan sesi WA-nya bulan ini sudah habis. Tawarkan upgrade ke Platinum (Rp 399rb/bulan) yang unlimited. Link: lamarcerdas.my.id/pricing`,
    abuse_warning: `User ini sudah kirim terlalu banyak pesan hari ini dan akan di-block 24 jam. Beritahu dengan cara yang tetap respectful.`,
  }
  return generateText({
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!sender || !message) return res.status(200).json({ status: 'ignored' })
    if (req.body.device === sender) return res.status(200).json({ status: 'ignored' })
    ...
    const { data: blacklist } = await supabase
      .from('wa_blacklist').select('id').eq('phone_wa', sender)
      .gte('blocked_until', new Date().toISOString()).limit(1)
    if (blacklist?.length > 0) return res.status(200).json({ status: 'blacklisted' })

    const { data: todayMessages } = await supabase
      .from('wa_conversations').select('id').eq('phone_wa', sender).eq('role', 'user').gte('created_at', today)
    ...
    const { data: userProfile } = await supabase
      .from('user_profiles').select('id, phone_verified').eq('phone_wa', sender).single()

    if (!userProfile) {
      const { data: prevReply } = await supabase
        .from('wa_conversations').select('id').eq('phone_wa', sender).eq('role', 'assistant').limit(1)
      const situation = (!prevReply?.length) ? 'unregistered_first' : 'unregistered_repeat'
    ...
    const { data: sub } = await supabase
      .from('subscriptions').select('plan').eq('user_id', userProfile.id).eq('status', 'active')
      .gte('expires_at', new Date().toISOString()).single()
    userPlan = sub?.plan || 'free'

    const { data: waUsage } = await supabase
      .from('wa_usage').select('id').eq('phone_wa', sender).eq('month_year', monthYear)
    ...
    if (userPlan === 'free') {
      const { data: prevGate } = await supabase
        .from('wa_conversations').select('id').eq('phone_wa', sender).eq('role', 'assistant')
        .gte('created_at', new Date(Date.now() - COOLDOWN_MINUTES * 60 * 1000).toISOString()).limit(1)
      const situation = (!prevGate?.length) ? 'free_gate_first' : 'free_gate_repeat'
      const reply = await generateMessage(situation, context)
      await saveAndSend(sender, message, reply, name)
    ...
    const { data: history } = await supabase
      .from('wa_conversations').select('role, content').eq('phone_wa', sender)
...
Kepribadian: hangat, genuine, seperti kakak perempuan senior yang peduli. Cerdas tapi tidak sok tahu.
...
Info paket: STARTER 10 sesi/Rp49rb | PRO 30 sesi/Rp199rb | PLATINUM unlimited/Rp399rb
...
User: paket ${userPlan} | sisa sesi WA: ${remaining} | nama: ${name || 'Kak'}`
    ...
    await saveAndSend(sender, message, reply, name)
    await supabase.from('wa_usage').insert({ phone_wa: sender, user_id: userProfile.id, month_year: monthYear })
